...
// Authentication error handler - now triggers full logout flow
const handleAuthError = (response: Response) => {
	if (response.status === 401) {
		// Clear invalid auth data immediately
		...
		// Force full page navigation to clear React state
...
export const createAssessment = async (assessmentData: CreateAssessmentRequest): Promise<Assessment> => {
	...
	if (!response.ok) {
		...
		throw new Error(extractErrorMessage(errorData, `HTTP Error: ${response.status}`));
	}

	const result: ApiResponse<Assessment> = await response.json();
	
	if (result.status === 'error') {
		throw new Error(extractErrorMessage(result, 'Failed to create assessment'));
	}
...
export const fetchAssessments = async (): Promise<Assessment[]> => {
	...
	if (result.status === 'error') {
		throw new Error(extractErrorMessage(result, 'Failed to fetch assessments'));
	}
...
export const fetchAssessment = async (id: number): Promise<Assessment> => {
	...
	if (result.status === 'error') {
		throw new Error(extractErrorMessage(result, 'Failed to fetch assessment'));
	}
...
export const deleteAssessment = async (id: number): Promise<any> => {
	...
	if (!response.ok) {
		handleAuthError(response);
		const errorData = await response.json().catch(() => ({ errors: ['Failed to delete assessment'] }));
		throw new Error(extractErrorMessage(errorData, `HTTP Error: ${response.status}`));
	}
	...
	if (result.status === 'error') {
		throw new Error(extractErrorMessage(result, 'Failed to delete assessment'));
	}
...
export const createAssessmentSection = async (assessmentId: number, sectionData: { name: string; order?: number }): Promise<any> => {
	...
	if (result.status === 'error') {
		throw new Error(extractErrorMessage(result, 'Failed to create section'));
	}
...
export const fetchAssessmentSections = async (assessmentId: number): Promise<any[]> => {
	...
	if (result.status === 'error') {
		throw new Error(extractErrorMessage(result, 'Failed to fetch sections'));
	}
...
// Update section
export const updateAssessmentSection = async (assessmentId: number, sectionId: number, sectionData: { name?: string; order?: number }): Promise<any> => {
	const token = getAuthToken();
	...
	if (!response.ok) {
		handleAuthError(response);
		const errorData = await response.json().catch(() => ({ errors: ['Failed to update section'] }));
		throw new Error(extractErrorMessage(errorData, `HTTP Error: ${response.status}`));
	}
	...
	if (result.status === 'error') {
		throw new Error(extractErrorMessage(result, 'Failed to update section'));
	}
...
export const deleteAssessmentSection = async (assessmentId: number, sectionId: number): Promise<any> => {
	...
	if (result.status === 'error') {
		throw new Error(extractErrorMessage(result, 'Failed to delete section'));
	}
...
export const createAssessmentQuestion = async (assessmentId: number, sectionId: number, questionData: any): Promise<any> => {
	...
	if (result.status === 'error') {
		throw new Error(extractErrorMessage(result, 'Failed to create question'));
	}
...
export const updateAssessmentQuestion = async (assessmentId: number, sectionId: number, questionId: number, questionData: any): Promise<any> => {
	...
	if (result.status === 'error') {
		throw new Error(extractErrorMessage(result, 'Failed to update question'));
	}
...
export const deleteAssessmentQuestion = async (assessmentId: number, sectionId: number, questionId: number): Promise<any> => {
	...
	if (result.status === 'error') {
		throw new Error(extractErrorMessage(result, 'Failed to delete question'));
	}
...
export const updateQuestionOption = async (questionId: number, optionId: number, optionData: any): Promise<any> => {
	...
	if (result.status === "error") {
		throw new Error(extractErrorMessage(result, "Failed to update option"));
	}
