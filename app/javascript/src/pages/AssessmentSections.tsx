...
import { fetchAssessment, fetchAssessmentSections, createAssessmentSection, updateAssessmentSection, createAssessmentQuestion, updateAssessmentQuestion, deleteAssessmentQuestion, updateQuestionOption, deleteAssessmentSection } from "../api/assessments";
...
export default function AssessmentSections() {
	...
	const [showAddQuestionModal, setShowAddQuestionModal] = useState<{ show: boolean; sectionId: number | null; sectionName: string }>({ show: false, sectionId: null, sectionName: '' });
	...
	const calculatePopoverPosition = (buttonElement: HTMLElement) => {
	...
	const handleUpdateSection = async (sectionId: number, updates: any) => {
		...
		try {
			await updateAssessmentSection(Number(id), sectionId, updates);
			// Reload sections to show updated section
	...
	const handleAddQuestion = (sectionId: number) => {
		...
		if (section) {
			setShowAddQuestionModal({ show: true, sectionId, sectionName: section.name });
		}
	};
	...
	const handleCreateQuestion = async (sectionId: number, questionType: string) => {
		...
		try {
			...
			setShowSaveBar(true);
			setShowAddQuestionModal({ show: false, sectionId: null, sectionName: '' });
		} catch (err) {
	...
	const handleDrop = async (e: React.DragEvent, dropTargetSectionId: number) => {
		e.preventDefault();
		
		if (!draggedSection || draggedSection === dropTargetSectionId) return;

		try {
			// Find the sections
			const draggedIdx = sections.findIndex(s => s.id === draggedSection);
			const targetIdx = sections.findIndex(s => s.id === dropTargetSectionId);
			
			if (draggedIdx === -1 || targetIdx === -1) return;

			// Reorder sections optimistically
			const newSections = [...sections];
			const [draggedSectionObj] = newSections.splice(draggedIdx, 1);
			newSections.splice(targetIdx, 0, draggedSectionObj);
			...
			// await updateSectionOrder(Number(id), newSections.map(s => ({ id: s.id, order: s.order })));
				...
				<div className="max-w-4xl mx-auto">
					<div className="space-y-6">
						...
						{sections.map((section, sectionIndex) => (
							<Card 
								key={section.id} 
								className={`shadow-none border border-gray-200 group transition-all ${
									draggedSection === section.id ? 'ring-2 ring-blue-400 bg-blue-50' : ''
								}`}
								draggable
								...
								<div className="p-6">
									...
									<div className="flex items-center justify-between mb-6">
										<div className="flex items-center space-x-3">
											<GripVertical className="w-4 h-4 text-gray-400 cursor-move hover:text-blue-500 transition-colors" />
											<div>
												{editingSection === section.id ? (
													<input
														type="text"
														defaultValue={section.name}
														className="text-lg font-semibold text-gray-900 bg-transparent border border-gray-300 rounded px-2 py-1 focus:border-purple-500 focus:ring-purple-500"
														onBlur={(e) => {
									...
									<div className="space-y-4">
										...
										{/* Add Question Button with Popover */}
										{section.questions && section.questions.length > 0 ? (
											<div className="pt-4 border-t border-gray-200 relative">
												<button 
													onClick={() => handleAddQuestion(section.id)}
													className="flex items-center px-4 py-2 text-sm font-medium transition-colors rounded-lg text-purple-600 bg-purple-50 border border-purple-200 hover:bg-purple-100"
												>
											...
											<div className="border-2 border-dashed border-gray-200 rounded-lg p-6 text-center">
												...
												<button 
													onClick={() => handleAddQuestion(section.id)}
													className="text-sm mt-2 text-purple-600 hover:underline transition-colors"
												>
													Add first question
												</button>
											</div>
						...
						<div className="text-center relative" ref={sectionPopoverRef}>
							<button 
								onClick={(e) => {
									...
									setShowSectionTypes(!showSectionTypes);
								}}
								...
								<div className={`absolute left-1/2 transform -translate-x-1/2 z-50 ${
									...
									<div className="bg-white rounded-lg shadow-xl border border-gray-200 p-4 min-w-[400px]">
										...
										<div className={`absolute left-1/2 transform -translate-x-1/2 ${
											...
											<div className={`w-4 h-4 bg-white border border-gray-200 transform ${
												popoverPosition === 'top'
						...
						<ConditionalLogicModal 
							show={showConditionalLogic !== null}
							onClose={() => setShowConditionalLogic(null)}
							sections={sections}
							questionId={showConditionalLogic?.questionId}
							sectionId={showConditionalLogic?.sectionId}
							onSave={(data) => {
								if (showConditionalLogic?.sectionId && showConditionalLogic?.questionId) {
									handleUpdateConditionalLogic(showConditionalLogic.sectionId, showConditionalLogic.questionId, data);
								}
							}}
						/>
						...
						{/* Add Question Modal */}
						<AddQuestionModal
							show={showAddQuestionModal.show}
							onClose={() => setShowAddQuestionModal({ show: false, sectionId: null, sectionName: '' })}
							sectionId={showAddQuestionModal.sectionId || 0}
							sectionName={showAddQuestionModal.sectionName}
