...
export default function AssessmentSections() {
	...
	// Click outside to close question popover
	useEffect(() => {
		function handleClickOutside(event: MouseEvent) {
									...
									<div className="space-y-4">
										...
										{section.questions && section.questions.length > 0 ? (
											<div className="pt-4 border-t border-gray-200 relative">
												<div ref={showQuestionTypes?.sectionId === section.id ? questionPopoverRef : null}>
													<button 
														onClick={() => handleAddQuestion(section.id)}
														className={`flex items-center px-4 py-2 text-sm font-medium transition-colors rounded-lg ${
															showQuestionTypes?.sectionId === section.id
																...
																<div className="grid grid-cols-2 gap-2 max-h-80 overflow-y-auto">
																	{availableQuestionTypes.map((type) => {
																		...
																		return (
																			<button
																				key={type.type}
																				onClick={() => handleCreateQuestion(section.id, type.type)}
																				className="p-3 rounded-lg border border-gray-200 hover:border-purple-300 hover:bg-purple-50 transition-all text-left group"
																			>
																				<div className="flex items-center space-x-3">
																					<div className={`w-8 h-8 rounded-lg flex items-center justify-center ${type.bgColor} group-hover:scale-105 transition-transform`}>
											...
											<div className="border-2 border-dashed border-gray-200 rounded-lg p-6 text-center relative">
												<div ref={showQuestionTypes?.sectionId === section.id ? questionPopoverRef : null}>
													...
													{/* Question Types Popover for Empty Section */}
													{showQuestionTypes?.sectionId === section.id && (
