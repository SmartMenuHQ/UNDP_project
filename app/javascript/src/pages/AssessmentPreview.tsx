...
export default function AssessmentPreview() {
	...
	const handleResponseChange = (questionId: number, value: any, type: 'option' | 'text' | 'number' | 'boolean') => {
		...
		const newVisibleSections = getVisibleSections(allSections as ConditionalSection[], newResponses);
	...
	const getConditionalStats = () => {
		...
		const visibleQuestions = visibleSections.reduce((sum, section) => sum + section.questions.length, 0);
		const conditionalQuestions = allSections.reduce((sum, section) => 
			sum + section.questions.filter(q => q.is_conditional).length, 0
		);
	...
	const renderQuestionPreview = (question: Question) => {
		...
		return (
			<div key={question.id} className="border border-gray-200 rounded-lg p-6 bg-white shadow-none">
		...
		<DashboardLayout>
			...
			<DashboardLayout.Content>
				<div className="max-w-4xl mx-auto">
					...
					{/* Assessment Header */}
					<div className="mb-8">
						<Card className="bg-gradient-to-br from-white to-gray-50 border border-gray-200 shadow-none">
							<div className="p-8">
